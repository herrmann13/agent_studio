...
static void webviewLoadChanged(WebKitWebView *web_view, WebKitLoadEvent load_event, gpointer data)
{
    ...
    {
        // WebKitGTK can restore the GTK drag destination during page load.
        // Remove it after loading so HTML5 drag events reach the web content.
        if (GPOINTER_TO_INT(data) == 0)
        {
            gtk_drag_dest_unset(GTK_WIDGET(web_view));
        }
...
GtkWidget *SetupWebview(void *contentManager, GtkWindow *window, int hideWindowOnClose, int gpuPolicy, int disableWebViewDragAndDrop, int enableDragAndDrop)
{
    ...
    g_signal_connect(G_OBJECT(webview), "load-changed", G_CALLBACK(webviewLoadChanged), GINT_TO_POINTER(enableDragAndDrop));

    // GTK registers the WebView as a drag destination by default, which
    // intercepts HTML5 drag events before WebKitGTK can dispatch them.
